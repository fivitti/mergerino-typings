 ...
 */
type FunctionPatch<T> = (val: T, merge: Merge<T extends object ? T : never>) => T;
 ...
 */
type ArrayPatch<T> = T extends Array<infer V> ? ObjectPatch<Record<number, V>> : never;
 ...
 */
type NestedPatch<T> = T extends object ? ObjectPatch<T> : never;
 ...
 */
type ObjectPatch<S extends object> = { [K in keyof S]?: S[K] | DeletePatch | FunctionPatch<S[K]> | NestedPatch<S[K]> | ArrayPatch<S[K]> };
 ...
 */
type TopLevelPatch<S extends object> = FunctionPatch<S> | ObjectPatch<S> | ArrayPatch<S> | Falsy;

/**
 * You can pass multiple patches in a single merge call, array arguments will
 * be flattened before processing.
 */
type MultipleTopLevelPatch<S extends object> = TopLevelPatch<S> | DeepArray<TopLevelPatch<S>>;
 ...
 */
type Merge<S extends object> = (source: S, ...patches: Array<MultipleTopLevelPatch<S>>) => S;
